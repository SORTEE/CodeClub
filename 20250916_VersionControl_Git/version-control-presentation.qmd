...
::: {.notes}
It's important to remember that while we're using GitHub today because it's the most popular and has great community features, it is not the only option.

There are other major platforms like GitLab and Bitbucket, which offer very similar features.

Crucially for researchers, your own university or institute might have its own private Git server, often running GitLab. This can be a huge advantage for projects with data privacy concerns.
